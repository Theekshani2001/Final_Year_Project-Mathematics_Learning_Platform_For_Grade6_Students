...
@Component({
  selector: 'app-e-division',
  templateUrl: './e-division.component.html',
  styleUrls: ['./e-division.component.scss']
})
export class EDivisionComponent implements OnInit {
  ...
  errorCount: number = 0;
  score: number = 0;
  private totalEquations: number = 2;
  private timer: any;
  ...
  generateEquation() {
    ...
    while (!validEquation) {
      this.num2 = this.getRandomNumber(11, 50); // Denominator (smaller range to avoid large values)
      this.num1 = this.getRandomNumber(this.num2 + 1, 100); // Numerator (must be larger than num2)
