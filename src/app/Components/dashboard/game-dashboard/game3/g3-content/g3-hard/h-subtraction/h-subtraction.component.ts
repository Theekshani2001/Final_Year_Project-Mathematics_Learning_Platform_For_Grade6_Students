...
@Component({
  selector: 'app-h-subtraction',
  templateUrl: './h-subtraction.component.html',
  styleUrls: ['./h-subtraction.component.scss']
})
export class HSubtractionComponent implements OnInit {
  ...
  generateOptions(correctAnswer: number): number[] {
    ...
    // Ensure that the generated options are positive numbers
    while (options.size < 3) {
      const randomAnswer = Math.max(0, this.getRandomNumber() - this.getRandomNumber());

      // Make sure the random answer is positive and not the same as the correct answer
      if (randomAnswer > 0 && randomAnswer !== correctAnswer) {
        options.add(randomAnswer);
      }
    }

    // Ensure that there are always 3 options (including the correct answer)
